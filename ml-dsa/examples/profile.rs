...
//!
//! Isolate one operation with `--collect-atstart=no` plus one `--toggle-collect`:
//!
...
/// `IntegerToBytes(0,1) || IntegerToBytes(0,1) || M`.
...
/// Print the deterministic rejection-loop iteration count for a batch of messages,
/// so a low- and high-iteration case can be picked for the profiling comparison.
...
fn scan() {
    ...
    // Also report the default profiling message for reference.
...
// --- Baseline operations (each named + non-inlined so the toggle always fires) ---
...
// --- Improved operations (Signer split so setup vs per-signature profile apart) ---
