//! Annotated KeyGen -> Sign -> Verify for ML-DSA-65.
//!
//! Full annotated demo (default message, or your own):
//!     cargo run --example roundtrip
//!     cargo run --example roundtrip -- "test message"
//!     cargo run --example roundtrip -- demo "test message" "test context"
//!
//! Sign as a standalone step (saves the public key + signature to files):
//!     cargo run --example roundtrip -- sign "test message" ["test context"]
//!
//! Verify as a standalone step:
//!     cargo run --example roundtrip -- verify "test message" ["test context"]
...
/// Deterministic key pair (fixed seed). Real use draws a random 32-byte seed.
fn demo_keypair() -> (Vec<u8>, Vec<u8>) {
    key_gen_internal(&[0x42u8; 32])
}

/// sign sub-command: keygen, sign, and save the public key + signature to files so verify can be run separately afterwards.
...
/// verify sub-command: read the saved public key + signature and check them against the given message/context.
...
fn do_demo(message: &str, context: &str) {
    ...
    // 4. Negative checks: any change will be rejected
