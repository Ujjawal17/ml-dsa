...
pub fn expand_a(rho: &[u8; 32]) -> PolyMatNTT<K, L> {
...
pub fn expand_s(rho: &[u8; 64]) -> (PolyVec<L>, PolyVec<K>) {
    let mut s1 = PolyVec::<L>::zero();
    let mut s2 = PolyVec::<K>::zero();
    for r in 0..L {
        s1.v[r] = rej_bounded_poly(&bounded_seed(rho, r as u64));
    }
    for r in 0..K {
        s2.v[r] = rej_bounded_poly(&bounded_seed(rho, (r + L) as u64));
    }
    (s1, s2)
}
...
pub fn expand_mask(rho: &[u8; 64], mu: u32) -> PolyVec<L> {
    let c = 1 + bitlen(GAMMA1 as u32 - 1) as usize; // 1 + 19 = 20
    ...
    for r in 0..L {
        ...
        y.v[r] = bit_unpack(&v, (GAMMA1 - 1) as u32, GAMMA1 as u32);
    }
    y
}
...
mod tests {
    ...
    fn expand_a_range_and_deterministic() {
        ...
        assert_eq!(a.rows[2].v[1].coeffs, expand_a(&rho).rows[2].v[1].coeffs);
    }
    ...
    fn expand_mask_in_range() {
        let rho = [2u8; 64];
        let y = expand_mask(&rho, 0);
        assert_eq!(y.v.len(), L);
        for p in y.v.iter() {
            assert!(p.coeffs.iter().all(|&x| (-(GAMMA1 - 1)..=GAMMA1).contains(&x)));
        }
    }
}
