...
use crate::params::{BETA, C_TILDE_BYTES, GAMMA1, GAMMA2, L, OMEGA};
...
pub fn sign_internal_traced(sk: &[u8], m_prime: &[u8], rnd: &[u8; 32]) -> (Vec<u8>, u32) {
    ...
    loop {
        ...
        let mut hc = H::init();
        hc.absorb(&mu);
        hc.absorb(&crate::serdes::w1_encode(&w1));
        let c_tilde = hc.finalize().squeeze_vec(C_TILDE_BYTES);
        ...
        let z = add_vec(&y, &cs1);
        let r0 = low_bits_vec(&sub_vec(&w, &cs2));

        // line 23: first validity check
        if inf_norm(&z) >= GAMMA1 - BETA || inf_norm(&r0) >= GAMMA2 - BETA {
            kappa += L as u32;
            continue;
        }
        ...
        let h = make_hint_vec(&neg_vec(&ct0), &r_arg);

        // line 28: second validity check
        if inf_norm(&ct0) >= GAMMA2 || count_ones(&h) > OMEGA {
            kappa += L as u32;
            continue;
        }
...
pub fn sign<R: CryptoRng + RngCore>(
    ...
    Ok(sign_internal(sk, &format_m_prime(ctx, m), &rnd))
}
...
mod tests {
    ...
    use crate::params::SIG_BYTES;

    #[test]
    fn sign_shapes_and_deterministic() {
        let (_pk, sk) = key_gen_internal(&[0x11u8; 32]);
        let sig = sign_deterministic(&sk, b"hello", b"").unwrap();
        assert_eq!(sig.len(), SIG_BYTES);
        // deterministic variant is reproducible
        assert_eq!(sig, sign_deterministic(&sk, b"hello", b"").unwrap());
    }

    #[test]
    fn ctx_too_long_is_rejected() {
        let (_pk, sk) = key_gen_internal(&[0x22u8; 32]);
        let long = vec![0u8; 256];
        assert!(matches!(sign_deterministic(&sk, b"m", &long), Err(Error::ContextTooLong)));
    }
}
