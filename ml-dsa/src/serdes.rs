...
use crate::params::{
    C_TILDE_BYTES, D, ETA, GAMMA1, GAMMA2, K, L, PK_BYTES, Q, SIG_BYTES, SK_BYTES,
};
...
/// Bytes per `s1`/`s2` polynomial under BitPack(η, η): `32 · bitlen(2η)` = 128.
const ETA_BYTES: usize = 32 * bitlen(2 * ETA as u32) as usize;
...
/// Bytes per `z` polynomial under BitPack(γ1−1, γ1): `32 · bitlen(2γ1−1)` = 640.
const Z_BYTES: usize = 32 * bitlen((GAMMA1 - 1) as u32 + GAMMA1 as u32) as usize;
/// Top of the `w1` coefficient range: `(q-1)/(2γ2) − 1` = 15.
const W1_TOP: u32 = ((Q - 1) / (2 * GAMMA2)) as u32 - 1;

/// FIPS 204, Algorithm 22 — pkEncode.
pub fn pk_encode(rho: &[u8; 32], t1: &PolyVec<K>) -> Vec<u8> {
...
pub fn pk_decode(pk: &[u8]) -> Result<([u8; 32], PolyVec<K>)> {
    if pk.len() != PK_BYTES {
        return Err(Error::InvalidLength { expected: PK_BYTES, got: pk.len() });
    }
    let mut rho = [0u8; 32];
...
type DecodedSk = ([u8; 32], [u8; 32], [u8; 64], PolyVec<L>, PolyVec<K>, PolyVec<K>);
...
pub fn sk_encode(
    ...
    let mut sk = Vec::with_capacity(SK_BYTES);
    sk.extend_from_slice(rho);
    sk.extend_from_slice(k_seed);
    sk.extend_from_slice(tr);
    for i in 0..L {
        sk.extend_from_slice(&bit_pack(&s1.v[i], ETA as u32, ETA as u32));
    }
    for i in 0..K {
        sk.extend_from_slice(&bit_pack(&s2.v[i], ETA as u32, ETA as u32));
    }
    for i in 0..K {
...
pub fn sk_decode(sk: &[u8]) -> Result<DecodedSk> {
    if sk.len() != SK_BYTES {
        return Err(Error::InvalidLength { expected: SK_BYTES, got: sk.len() });
    }
    let mut rho = [0u8; 32];
    ...
    for i in 0..L {
        s1.v[i] = bit_unpack(&sk[off..off + ETA_BYTES], ETA as u32, ETA as u32);
        off += ETA_BYTES;
    }
    let mut s2 = PolyVec::<K>::zero();
    for i in 0..K {
        s2.v[i] = bit_unpack(&sk[off..off + ETA_BYTES], ETA as u32, ETA as u32);
        off += ETA_BYTES;
    }
...
pub fn sig_encode(c_tilde: &[u8], z: &PolyVec<L>, h: &PolyVec<K>) -> Vec<u8> {
    let mut sigma = Vec::with_capacity(SIG_BYTES);
    sigma.extend_from_slice(c_tilde);
    for i in 0..L {
        sigma.extend_from_slice(&bit_pack(&z.v[i], (GAMMA1 - 1) as u32, GAMMA1 as u32));
    }
...
pub fn sig_decode(sigma: &[u8]) -> Result<(Vec<u8>, PolyVec<L>, PolyVec<K>)> {
    if sigma.len() != SIG_BYTES {
        return Err(Error::InvalidLength { expected: SIG_BYTES, got: sigma.len() });
    }
    let c_tilde = sigma[..C_TILDE_BYTES].to_vec();
    let mut off = C_TILDE_BYTES;
    let mut z = PolyVec::<L>::zero();
    for i in 0..L {
        z.v[i] = bit_unpack(&sigma[off..off + Z_BYTES], (GAMMA1 - 1) as u32, GAMMA1 as u32);
        off += Z_BYTES;
    }
    let h = hint_bit_unpack(&sigma[off..]).ok_or(Error::Reject)?;
    Ok((c_tilde, z, h))
}

/// FIPS 204, Algorithm 28 — w1Encode.
pub fn w1_encode(w1: &PolyVec<K>) -> Vec<u8> {
    let mut out = Vec::new();
    for i in 0..K {
        out.extend_from_slice(&simple_bit_pack(&w1.v[i], W1_TOP));
    }
    out
}
...
mod tests {
    use super::*;
    use crate::params::C_TILDE_BYTES;
    ...
    fn pk_round_trip() {
        ...
        let (rho2, t12) = pk_decode(&pk).unwrap();
        assert_eq!(rho2, rho);
        for i in 0..K {
    ...
    fn sk_round_trip() {
        ...
        let sk = sk_encode(&rho, &k_seed, &tr, &s1, &s2, &t0);
        assert_eq!(sk.len(), SK_BYTES);
        let (r2, k2, tr2, s1b, s2b, t0b) = sk_decode(&sk).unwrap();
    ...
    fn sig_round_trip() {
        ...
        let c_tilde = vec![0x5Au8; C_TILDE_BYTES];
        let mut z = PolyVec::<L>::zero();
        for p in z.v.iter_mut() {
            for c in p.coeffs.iter_mut() {
                *c = rng.range(-(GAMMA1 - 1), GAMMA1);
            }
        }
    ...
    fn decoders_reject_wrong_length() {
        assert!(matches!(pk_decode(&[0u8; 10]), Err(Error::InvalidLength { .. })));
        assert!(matches!(sk_decode(&[0u8; 10]), Err(Error::InvalidLength { .. })));
        assert!(matches!(sig_decode(&[0u8; 10]), Err(Error::InvalidLength { .. })));
    }
}
