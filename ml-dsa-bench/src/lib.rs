...
//! The reference and improved paths are byte-identical (signing same messages under same key) and the two hit the same rejection-loop counts.
//! Difference in instruction counts is then purely the optimization.
...
/// `M'` for the pure variant, empty context.
